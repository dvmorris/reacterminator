...
function htmlToHtmlSnippets (html, options) {
  ...
  if (options.fileToComponent) {
    const fileNameWithoutExt = path.parse(options.name).name
    const fileComponentName = upperCamelCase(fileNameWithoutExt)
  ...
  $('[data-component-name]').each(function () {
    const htmlSnippet = elementToHtmlSnippet($(this), $, htmlSnippets)
    if (htmlSnippet) {
...
function elementToHtmlSnippet ($element, $, htmlSnippets, options) {
  ...
  const component = Object.assign({}, options, new Attr($element).extract())
