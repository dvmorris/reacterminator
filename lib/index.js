...
function reacterminator ({type, content}, options) {
  ...
  const mergedFileSnippets = _.reduce(
    htmlFiles,
    function (acc, {htmlFile, name}) {
      const htmlToReactOptions = _.extend({name}, options)
  ...
  const formattedFileSnippets = _.mapValues(mergedFileSnippets, formatFileSnippet)
