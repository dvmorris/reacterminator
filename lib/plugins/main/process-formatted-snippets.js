...
module.exports = function processFormattedSnippets ({components, options}) {
  const readonlyComponents = _.omitBy(components, 'isCustom')
  const customComponents = _.pickBy(components, 'isCustom')
  ...
  createComponentFiles({
    ...
    outputPath: `${options.outputPath}/custom-components`,
    override: false
  })
...
function writeToFile ({component, outputPath, override}) {
  const {name, formattedFileSnippet} = component
  const filePath = `${outputPath}/${name}.jsx`
